...
int main(int argc, char *argv[]) {
  ...
  //determinant print threshold
    ...
    std::vector<unsigned> new_orbs; //change the name of the orbitals
    ...
    for (int i = 1; i < argc - 1; ++i) {
      ...
      if (std::string(argv[i])== "--norbs"){ // number of orbitals
          ...
          printf("\n");
          //somehow we need to make 1->0, 2->1, 3->2, ..., 0->norb-1 to make the first orbital correct
          for (unsigned orb : orbs) {
              unsigned mapped = (orb == 0) ? norbs - 1 : orb - 1;
              new_orbs.push_back(mapped);
          }
          // Print the list
          printf("renewed orbital list by a cyclic maping: ");
