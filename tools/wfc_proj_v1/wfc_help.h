...
LambdaResult Compute_Lambda(
    ...
    for (size_t j = 0; j < n_det; ++j){
        int ne_s_up = 0; // number of electrons in A_S, up
        int ne_s_dn = 0; // number of electrons in A_S, dn
        const Det &det = wf_l.dets[j];
        std::vector<int> overlap_list_up;
    ...
    printf("   - 0e transfer (G_S):         %d\n", count_Gs);
    printf("   - 1e transfer:               %d\n", count_1e_transfer);
    printf("   - 2e transfer:               %d\n", count_2e_transfer);
    printf("   - 3e transfer:               %d\n", count_3e_transfer);
    printf("   - 4e transfer:               %d\n", count_4e_transfer);
